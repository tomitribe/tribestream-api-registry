...
public class WithElasticSearchCallInterceptor {
    ...
    private boolean isUnreachableException(Throwable original) {
        if (UnreachableException.class.isInstance(original)) {
            return true;
        }
        ...
        return isUnreachableException(cause);
    }
    ...
    public Object onCall(InvocationContext ctx) throws Exception {
        try {
            ...
            if (isUnreachableException(original)) {
                return Response.status(Response.Status.SERVICE_UNAVAILABLE)
                        .entity("{\"key\": \"elasticsearch.unavailable.exception\"}")
                        .build();
            }
            throw original;
        }
